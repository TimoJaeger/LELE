...
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#----------------------------------------Track Analyses at Border Transect Level---------------------------------------
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
# Inner shared borders list the same individual as ingoing and outgoing --> double counted! (adjustment necessary for later analyses)
...
TransSuit <- aggregate(x = TransectSuitability$TransectSuit, 
                                       by = list(TransectSuitability$Block,
                                                 TransectSuitability$BorderAbsolute,
                                                 TransectSuitability$Fencetype), 
                                       FUN = mean, na.rm = TRUE)
colnames(TransSuit) <- c("Block", "BorderAbsolute", "Fencetype", "TransectSuit")

# add 1 for "Bordercount"
TransSuit$Bordercount <- 1

# sum up data
TransSuit <- aggregate(x = cbind(TransSuit$TransectSuit, TransSuit$Bordercount), 
                       by = list(TransSuit$Block, TransSuit$Fencetype), 
                       FUN = sum, na.rm = TRUE)
colnames(TransSuit) <- c("Block",  "Fencetype", "TransectSuit", "Bordercount")

# mean of TransectSuit (per Border)
TransSuit$TransectSuitmean <- TransSuit$TransectSuit/TransSuit$Bordercount

# exclude fullex Borders as not included in models for tracks
TransSuit <- TransSuit[!TransSuit$Fencetype == "fullex",]
...
# inner block borders are shared --> a control plot bordering a full exclosure plot has a full exclosure fence as border!! --> unequal numbers per treatment

# Border transect analyses were conducted independent of the plot
# The natural logarithm of the number of similar transects was used as an offset to account for unequal transect numbers (see Models)

## Notes regarding Track direction
# Track direction can be "Ingoing", "Outgoing" and "Other"
# Directions used for Border Transect Analysis are "Ingoing" and "Outgoing". Both are used as not possible to separate for borders inside block (=borders shared by neighbouring plots)
# However, to prevent double counting of the tracks at inner borders, halving the values is necessary!! This is not important for the plot analysis
...
for (i in 1:length(ListTracks_all)) {
  ...
  # sum up tracks per Border --> inner Borders have all values double!! 
  Tracks_all <- aggregate(IndNumbers ~ BorderAbsolute, data = Tracks_all, FUN = sum, na.rm = TRUE)
  ...
  # delete rows where "BorderAbsolute" is double in the data
  Tracksanalysis <- aggregate(x = Tracksanalysis$IndNumbers, by = list(Tracksanalysis$Block, 
                                                                       Tracksanalysis$BorderAbsolute,
                                                                       Tracksanalysis$Fencetype,
                                                                       Tracksanalysis$BorderNumber),
                              FUN = mean, na.rm = FALSE)
  colnames(Tracksanalysis) <- c("Block", "BorderAbsolute", "Fencetype", "BorderNumber", "AnimalNumber")
  
  # cut values of inner borders in half to prevent double counting
  Tracksanalysis[Tracksanalysis$BorderNumber %in% c(11:17),5] <- Tracksanalysis[Tracksanalysis$BorderNumber %in% c(11:17),5]/2
  
  # how many borders for each Block-Fencetype combination? Necessary because of unequal numbers!
  # calculate sum for each Block and Treatment
  Tracksanalysis$Bordercount <- 1
  Tracksanalysis <- aggregate(x = cbind(Tracksanalysis$AnimalNumber, Tracksanalysis$Bordercount),
                              by = list(Tracksanalysis$Block, Tracksanalysis$Fencetype),
                              FUN = sum, na.rm = FALSE)
  colnames(Tracksanalysis) <- c("Block", "Fencetype", "AnimalNumber", "Bordercount")
...
# calculate mean per Border
ListallDatamean <- list()
ListallDatamean <- ListallData

for (i in 1:length(ListallData)){
  ListallDatamean[[i]]$AnimalNumber <- ListallDatamean[[i]]$AnimalNumber/ListallDatamean[[i]]$Bordercount
}
...
# pdf(file="Boxplots_Borders.pdf",width=16,height=9)
for (i in 1:length(ListallDatamean)){
  ...
  plot(Tracksanalysis$AnimalNumber ~ Tracksanalysis$Fencetype, ylab = "Number of Tracks", xlab= "Fencetype", 
       main = paste(NAMES[i], "allBorders", sep =" "))
...
# x11(16,9)
# save as pdf (remove "#" in following line)
# pdf(file="Interaction_Borders.pdf",width=16,height=9)
for (i in 1:length(ListallDatamean)){
  if(i %in% c(1,10)){par(mfrow=c(3,3))}
  
  Split <- paste(str_split_fixed(names(ListallDatamean)[i], "_",2)[2], "allBorders", sep=" ")
...
#   an offset for the number of Borders is needed to make values comparable!!
...
for (i in c(1:length(ListallData))){
  ...
  # Glmm with glmmTMB
  TMB.nb <- glmmTMB(AnimalNumber ~ offset(log(Bordercount)) + Fencetype +(1|Block), 
  ...
  TMB.nb2 <- glmmTMB(AnimalNumber ~ offset(log(Bordercount)) + TransectSuit + Fencetype + (1|Block), 
                     data = Data, family = "nbinom2")
  summary(TMB.nb2)
...
# convergence problems: Duiker (TMB2), Wildebeest (TMB,TMB2), Giraffe (TMB2), s.Bovids (TMB), Eland (TMB,TMB2), Mega-Herbivores (TMB2)
...
for (i in 1:length(NAMES)){
  ...
  pvaluesallmodels[i,3] <- summary(TMBTsuit.nb)$coefficients$cond[rownames(summary(TMBTsuit.nb)$coefficients$cond) =="Fencetypemhex",4]
...
# save as xlsx (remove "#" in following line)
# write.xlsx(meanstrack, file = "dataoverview.xlsx", sheetName="tracksBorders", append=TRUE)
...
dispzerofunction <- function(s1){
  ...
  for (i in 1:length(ListallglmmTMB)){
    ...
    if(i %in% c(4,7,13,16)){
      ...
      TMBTsuit.nb <- glmmTMB(AnimalNumber ~  offset(log(Bordercount)) + TransectSuit + Fencetype + (1|Block), 
                            data = Data, family = "nbinom2")
    }else{
      TMBTsuit.nb <- TMB.nb
    }

...
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#------------------------------------------Track Analyses at Outer Borders Only----------------------------------------
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
# only outer borders relevant
TransSuit <- TransectSuitability[TransectSuitability$BorderINorOUT=="out",]

# calculate mean of identical "BorderAbsolute" (to prevent double counting of borders)
TransSuit <- aggregate(x = TransSuit$TransectSuit, 
                       by = list(TransSuit$Block,
                                 TransSuit$BorderAbsolute,
                                 TransSuit$Fencetype), 
                       FUN = mean, na.rm = TRUE)
...
# Directions used for Outer Border Transect Analysis are "Ingoing" only!
# Only ingoing tracks to investigate if animals are influenced by the fences when they make first contact while entering the block from outside
...
for (i in 1:length(ListTracks_all)) {
  ...
  # only outer edges
  Tracks_all <- Tracks_all[Tracks_all$BorderINorOUT == "out",]
  
  # only "ingoing" animals
  Tracks_all <- Tracks_all[Tracks_all$DirectionTrackName == "Ingoing",]
  ...
  # only outer borders
  Tracksanalysis <- Tracksanalysis[Tracksanalysis$BorderINorOUT=="out",]
  ...
  Tracksanalysis <- aggregate(x = cbind(Tracksanalysis$IndNumbers, Tracksanalysis$Bordercount),
...
# pdf(file="Boxplots_OuterBorders.pdf",width=16,height=9)
for (i in 1:length(ListallDatamean)){
  ...
  plot(Tracksanalysis$AnimalNumber ~ Tracksanalysis$Fencetype, ylab = "Number of Tracks", xlab= "Fencetype", 
       main = paste(NAMES[i], "OuterBorders", sep =" "))
...
# x11(16,9)
# save as pdf (remove "#" in following line)
# pdf(file="Interaction_OuterBorders.pdf",width=16,height=9)
for (i in 1:length(ListallDatamean)){
  if(i %in% c(1,10)){par(mfrow=c(3,3))}
  
  Split <- paste(str_split_fixed(names(ListallDatamean)[i], "_",2)[2], "OuterBorders", sep=" ")
...
# convergence problems: Duiker (TMB2), Eland (TMB), Wildebeest (TMB2), Giraffe (TMB2), s.Bovids (TMB),  Mega-Herbivores (TMB2), others (TMB)
...
# save as xlsx (remove "#" in following line)
# write.xlsx(meanstrack, file = "dataoverview.xlsx", sheetName="tracksOuterBorders", append=TRUE)
...
# RE not normal for Elephant, Eland, Carnivores
